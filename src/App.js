...
function App() {
  ...
  const doAPI = async () => {
    setIsLoading(true)
    let url = `http://www.omdbapi.com/?s=${search}&apikey=46cf0447`
  ...
  return (
    <Router>
      <div className="container bg-light  position-relative ">
        ...
        {isLoading ? <h2 className='display-1 position-absolute top-50 start-50'>{LoadingMessage}</h2> :
          <Switch>
        ...
        <div style={{ height: "60px" }} className='d-flex container bg-secondary align-items-center justify-content-start  text-light'>
          <span className='mx-3'>Muli</span>
          <hr/>
          <a className='mx-3' href='https://github.com/stft-ma20/STFT-p3-a1'>
