    ...
    <div className="row bg-secondary">
      <div className='d-flex align-items-center mx-1 text-light '>
        ...
        <div className='d-flex align-items-center justify-content-start mx-1'>
          ...
          <select selected="Title" className='form-select w-auto m-1 text-start' ref={sortByRef} onChange={() => { props.sortMedia(sortByRef.current.value) }}>
      ...
      <div className='row mx-1 justify-content-between' ref={yearBtnsRef}>
        ...
        {yearsArr.map(
          (item, index) => {
            return (
              <Link to={"/year/" + item} key={item} className='col col-sm-1 btn text-light bg-black m-2 py-0' onClick={() => { props.setYear(item) }}> {item}</Link>
            )
          }
        )
        }
